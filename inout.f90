...
!**********************************************************************
    ...
    ! If ROOTBETA is give, assign FRACROOT using Jackson's root distribution model.
...
!                     T0,TREF,TMAX,SMD1,SMD2,VK1,VK2,WLEAF,NSIDES,SWPEXP
    ...
    IF(IOERROR.NE.0) THEN
        close(ufile)
        CALL SUBERROR('ERROR: namelist BBGSCON was not red properly', IFATAL,IOERROR)
    ENDIF  
...
!***********************************************************
      SUBROUTINE InputDisag( &
        OpMie, OpRayleigh, OpEphrat, OpSpitters, &
        c, taun, p1, p2, Ik)
! Reads in the parameters and option-variables for weather disaggregation and growth        
!***********************************************************
...
! Get the default values so that Maestra runs without modifications in case file is missing 
    OpEphrat = 0
    OpSpitters = 0
    OpMie = 0
    OpRayleigh = 0
...
! Read in the options used for weather disaggregation
    READ(UGROWTH, nml = OpWeather, IOSTAT = IOERROR)
! Give a warning that default values are being used 
    IF (IOERROR.NE.0)  &
      CALL SUBERROR('WARNING: The default weather options are being used',IWARN,IOERROR)
! Read in the parameters used for weather disaggregation
    REWIND(UGROWTH)
    READ(UGROWTH, nml = ParWeather, IOSTAT = IOERROR)
! Give a warning that default values are being used 
    IF (IOERROR.NE.0)  &
      CALL SUBERROR('WARNING: The default weather parameters are being used',IWARN,IOERROR)
