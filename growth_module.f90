...
Namelist /growth_parameters/ RmRef_leaf, RmRef_shoots, RmRef_stem, RmRef_froots, RmRef_croots, RmRef_fruits, PC_leaf, PC_shoots, PC_stem, PC_froots, PC_croots, PV_leaf, PV_shoots, PV_stem, PV_froots, PV_croots, LAD, Q10, cp, Hmax, DensOpt, DOYPhen1, DOYPhen2, DOYPhen3, DOYPhen4, Kroots, PVfr, PVoil, Adult, DOYsenescence1, DOYsenescence2, PVres, CCres, PCfr, PCoil, OptPrun, d_alley, d_row, specific_leaf_area, RmRef_reserves, ThermalTimeRequirement, ColdRequirement, Phen_T0, Phen_Tx, Phen_a, Phen_Tb, ActiveWood

Namelist /growth_initial/ Biomass_leaf, Biomass_shoots, Biomass_stem, Biomass_froots, Biomass_croots, Biomass_fruits, Volume, LAI, H, Rx, Ry, Reserves, ReservesT, Biomass_leaf0, Biomass_leaf1, Biomass_leaf2, Biomass_leaf2T, Biomass_shoots0, Biomass_shoots1, Biomass_shoots2, Biomass_shoots2T, Age, ChillingHours, ThermalTime
...
! Open the output file called 'out_growth.dat'
    ...
    ENDIF
    Write(out_growth, '(24(A14,3x))') 'Year','DOY','Leaf','Shoots','Stem','Fine_roots','Coarse_roots','Fruits','Reserves','Volume','LAI','Height','Radius_x','Radius_y', 'Assimilation','M.Respiration','Leaf_0','Leaf_1','Leaf_2', 'Shoots_0', 'DeltaB_above', 'DeltaB_below','aPAR','PAR'
end subroutine read_growth_inputs

subroutine write_growth_outputs(Year, DOY, Assimilation, RmD, DeltaB_above, DeltaB_below, aPAR, PAR)
USE maestcom, only: out_growth
Implicit None
integer, intent(in) :: Year, DOY
double precision, intent(in) :: DeltaB_above, DeltaB_below, Assimilation, RmD, aPAR
real, intent(in) :: PAR
	Write(out_growth, '(I2,3x,I3,2x,22(ES16.6E3,3x))') Year, DOY, Biomass_leaf, Biomass_shoots, Biomass_stem, Biomass_froots, Biomass_croots, Biomass_fruits, Reserves, Volume, LAI, H, Rx, Ry, Assimilation, RmD, Biomass_leaf0, Biomass_leaf1, Biomass_leaf2, biomass_shoots0, DeltaB_above, DeltaB_below, aPAR, PAR
end subroutine
